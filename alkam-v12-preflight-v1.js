(function(){
  'use strict';
  var VERSION='ALKAM v12 Preflight v1.1';
  ...
  function scan(){
    ...
    items.push(safe('Sonuç Export',function(){return window.ALKAM_V12_PREFLIGHT_EXPORT_V1&&ALKAM_V12_PREFLIGHT_EXPORT_V1.test?ALKAM_V12_PREFLIGHT_EXPORT_V1.test():null}));
    ...
    var exportBad=items.some(function(x){return x.label==='Sonuç Export'&&x.result&&(!x.result.hasPreflight||!x.result.hasLiveTest||!x.result.hasWriteGate)});
    var ready=!failed&&!writeOpen&&!missing&&!liveBad&&!exportBad;
    var decision=ready?'v12 stabilizasyonuna geçilebilir':(writeOpen?'DUR: Supabase yazma açık':(exportBad?'Sonuç Export eksik':'Önce eksikler düzeltilecek'));
    var result={version:VERSION,ready:ready,decision:decision,failed:failed,writeOpen:writeOpen,missing:missing,liveBad:liveBad,exportBad:exportBad,items:items,time:new Date().toISOString()};
  ...
  function render(){
    ...
    body.innerHTML='<div class="alkam-preflight-note '+(r.ready?'ok':'bad')+'">Karar: '+r.decision+'</div><div class="alkam-preflight-actions"><button onclick="window.ALKAM_V12_PREFLIGHT_V1&&ALKAM_V12_PREFLIGHT_V1.render()">Yenile</button><button onclick="window.ALKAM_V12_PREFLIGHT_EXPORT_V1&&ALKAM_V12_PREFLIGHT_EXPORT_V1.open&&ALKAM_V12_PREFLIGHT_EXPORT_V1.open()">Sonuç Export Aç</button><button onclick="window.ALKAM_CANLI_TEST_PAKETI_V11&&ALKAM_CANLI_TEST_PAKETI_V11.open&&ALKAM_CANLI_TEST_PAKETI_V11.open()">Canlı Test Aç</button></div><div class="alkam-preflight-grid"><div class="alkam-preflight-card"><b>Ready</b><span>'+(r.ready?'Evet':'Hayır')+'</span></div><div class="alkam-preflight-card"><b>Failed</b><span>'+r.failed+'</span></div><div class="alkam-preflight-card"><b>Write Open</b><span>'+(r.writeOpen?'Evet':'Hayır')+'</span></div><div class="alkam-preflight-card"><b>Live Bad</b><span>'+(r.liveBad?'Evet':'Hayır')+'</span></div><div class="alkam-preflight-card"><b>Export Bad</b><span>'+(r.exportBad?'Evet':'Hayır')+'</span></div></div><div class="alkam-preflight-table"><table><thead><tr><th>Kontrol</th><th>Durum</th><th>Özet</th></tr></thead><tbody>'+r.items.map(function(x){var ok=x.ok&&x.result;return '<tr><td>'+x.label+'</td><td class="'+(ok?'alkam-preflight-ok':'alkam-preflight-bad')+'">'+(ok?'OK':'Eksik')+'</td><td><pre style="white-space:pre-wrap;margin:0;font-family:inherit">'+JSON.stringify(x.result||x.error||null,null,2).slice(0,900)+'</pre></td></tr>'}).join('')+'</tbody></table></div>';
    return r;
  }
