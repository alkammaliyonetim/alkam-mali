(function(){
  'use strict';
  var VERSION='ALKAM Professional UI v1.2 focus-layout';
  ...
  function css(){
    ...
    st.textContent='body{background:#f4f7fb!important;color:#0f172a!important}#alkamCariCoreBanner,#alkamDataQualityPanel,#alkamTahakkukPanel,#alkamTahakkukControlPanel{display:none!important}.alkam-ux-compact #alkamLastAmountsPanel{display:none!important}.alkam-ux-compact #alkamCariDetailStandardPanel{display:none!important}.topbar,.top-nav,[class*=topbar],[class*=top-nav]{min-height:42px!important;height:42px!important;overflow:hidden!important;box-shadow:0 8px 22px rgba(15,23,42,.06)!important;border-bottom:1px solid #dbe4f0!important}.topbar button,.top-nav button,[class*=topbar] button,[class*=top-nav] button{height:28px!important;font-size:11px!important;padding:0 9px!important}.topbar button:not(:first-child),.top-nav button:not(:first-child),[class*=topbar] button:not(:first-child),[class*=top-nav] button:not(:first-child){opacity:.45!important}.sidebar,[class*=sidebar]{box-shadow:8px 0 26px rgba(15,23,42,.10)!important}.nav-btn,.menu-item,.sidebar button{border-radius:12px!important;font-weight:900!important}.card,.metric-mini,[class*=card]{box-shadow:0 8px 24px rgba(15,23,42,.04)!important;border-color:#dbe4f0!important}#selectedCariDetail,[id*=selectedCari],.cari-detail,.detail-panel{background:#fff!important;border:1px solid #dbe4f0!important;border-radius:20px!important;box-shadow:0 14px 34px rgba(15,23,42,.06)!important}.alkam-ux-compact .alkam-son-bakiye-card{min-height:185px!important;border-width:2px!important;background:linear-gradient(180deg,#f8fbff,#eaf2ff)!important}.alkam-ux-compact .alkam-son-bakiye-title{font-size:13px!important}.alkam-ux-compact .alkam-son-bakiye-value{font-size:58px!important}#alkamProfessionalControlButton{position:fixed;right:22px;bottom:22px;z-index:999999;border:0;border-radius:999px;background:#071f49;color:#fff;height:46px;padding:0 17px;font-weight:950;box-shadow:0 18px 42px rgba(15,23,42,.28);cursor:pointer}#alkamProfessionalDrawer{position:fixed;right:22px;top:86px;bottom:78px;width:min(560px,calc(100vw - 44px));z-index:999998;background:#fff;border:1px solid #dbe4f0;border-radius:20px;box-shadow:0 26px 70px rgba(15,23,42,.26);display:none;overflow:auto;font-family:Arial,Helvetica,sans-serif}#alkamProfessionalDrawer.open{display:block}.alkam-drawer-head{padding:16px 18px;border-bottom:1px solid #dbe4f0;background:#f8fbff;display:flex;justify-content:space-between}.alkam-drawer-title{font-size:18px;font-weight:950;color:#0f172a;margin:0}.alkam-drawer-sub{font-size:12px;font-weight:800;color:#64748b;margin-top:5px}.alkam-drawer-close{width:34px;height:34px;border:0;border-radius:10px;background:#e8eef9;font-weight:950}.alkam-drawer-body{padding:14px 18px}.alkam-control-card{border:1px solid #e2e8f0;background:#fbfdff;border-radius:14px;padding:12px;margin-bottom:10px}.alkam-control-card b{display:block;font-size:13px;color:#0f172a;margin-bottom:7px}.alkam-control-card .line{font-size:12px;font-weight:800;color:#334155;line-height:1.55}.alkam-drawer-actions{display:flex;gap:8px;flex-wrap:wrap;margin-top:9px}.alkam-drawer-actions button{height:32px;border:0;border-radius:10px;background:#1769e8;color:#fff;font-weight:950;padding:0 10px;cursor:pointer}.alkam-drawer-actions button.secondary{background:#e8eef9;color:#0f172a}.alkam-ui-section-label{font-size:11px!important;font-weight:950!important;color:#8aa0bd!important;text-transform:uppercase!important;letter-spacing:.08em!important;margin:12px 10px 6px!important;padding-top:8px!important;border-top:1px solid rgba(255,255,255,.10)!important}.alkam-focus-note{display:inline-flex!important;align-items:center!important;gap:6px!important;border-radius:999px!important;background:#ecfdf5!important;color:#047857!important;border:1px solid #bbf7d0!important;padding:6px 10px!important;font-size:11px!important;font-weight:950!important;margin-left:8px!important}@media(max-width:900px){#alkamProfessionalDrawer{left:12px;right:12px;top:70px;bottom:72px;width:auto}.topbar,.top-nav,[class*=topbar],[class*=top-nav]{height:auto!important;min-height:42px!important}.alkam-ux-compact .alkam-son-bakiye-value{font-size:40px!important}}';
  ...
  function compactTopNav(){
    ...
    items.forEach(function(x){var t=(x.textContent||'').trim();if(/stok|fatura|çek|senet|muhasebe|araçlar|ayarlar|otomasyon/i.test(t))x.style.opacity='.45'});
  }
  function addFocusBadge(){
    var h=q('h1')||q('h2'); if(!h||q('#alkamFocusBadge'))return;
    var b=document.createElement('span'); b.id='alkamFocusBadge'; b.className='alkam-focus-note'; b.textContent='Sade görünüm aktif';
    h.appendChild(b);
  }
  function render(){
  ...
  function run(){
    css(); document.body.classList.add('alkam-ux-compact'); groupSidebar(); compactTopNav(); addFocusBadge();
  ...
  window.ALKAM_PROFESSIONAL_UI_V1={version:VERSION,run:run,open:function(){run();q('#alkamProfessionalDrawer').classList.add('open');render()},test:function(){return {version:VERSION,button:!!q('#alkamProfessionalControlButton'),drawer:!!q('#alkamProfessionalDrawer'),compact:document.body.classList.contains('alkam-ux-compact'),lastAmountsHidden:getComputedStyle(q('#alkamLastAmountsPanel')||document.body).display==='none',time:new Date().toISOString()}}};
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',run);else run();
  setInterval(function(){groupSidebar();compactTopNav();addFocusBadge();render()},4000);
})();
