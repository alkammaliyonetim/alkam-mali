(function(){
  'use strict';
  var VERSION='ALKAM Cache Deploy Kontrol v11.19';
  ...
  function scan(){
    ...
    var result={version:VERSION,expected:EXPECTED,status:missing?'Cache/Deploy Kontrol':'Güncel',missing:missing,checks:checks,time:new Date().toISOString()};
  ...
  function css(){
    ...
    st.textContent='.alkam-cache-modal{position:fixed;inset:0;z-index:1000030;background:rgba(15,23,42,.42);display:none;align-items:center;justify-content:center;padding:18px}.alkam-cache-modal.open{display:flex}.alkam-cache-box{width:min(880px,100%);max-height:88vh;background:#fff;border-radius:20px;border:1px solid #dbe4f0;box-shadow:0 30px 80px rgba(15,23,42,.32);font-family:Arial,Helvetica,sans-serif;overflow:hidden}.alkam-cache-head{padding:16px 18px;background:#f8fbff;border-bottom:1px solid #e2e8f0;display:flex;justify-content:space-between}.alkam-cache-head b{font-size:18px;color:#0f172a}.alkam-cache-head small{display:block;margin-top:4px;color:#64748b;font-weight:800}.alkam-cache-close{width:34px;height:34px;border:0;border-radius:10px;background:#e8eef9;font-weight:950;cursor:pointer}.alkam-cache-body{padding:16px 18px;overflow:auto;max-height:calc(88vh - 72px)}.alkam-cache-actions{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:12px}.alkam-cache-actions button{height:34px;border:0;border-radius:10px;background:#1769e8;color:#fff;font-weight:950;padding:0 10px;cursor:pointer}.alkam-cache-note{background:#fff7ed;border:1px solid #fed7aa;color:#9a3412;border-radius:12px;padding:10px;margin-bottom:12px;font-weight:900}.alkam-cache-table{border:1px solid #e2e8f0;border-radius:14px;overflow:hidden}.alkam-cache-table table{width:100%;border-collapse:collapse}.alkam-cache-table th{background:#f8fafc;color:#64748b;font-size:11px;text-transform:uppercase;text-align:left;padding:9px}.alkam-cache-table td{border-top:1px solid #eef2f7;padding:9px;font-size:12px;font-weight:800}.alkam-cache-ok{color:#047857;font-weight:950}.alkam-cache-bad{color:#b91c1c;font-weight:950}@media(max-width:900px){.alkam-cache-table{overflow:auto}.alkam-cache-table table{min-width:720px}}';
  ...
  function modal(){
    ...
    el.innerHTML='<div class="alkam-cache-box"><div class="alkam-cache-head"><div><b>Cache / Deploy Kontrol</b><small>Canlı sayfada yeni sürüm gerçekten geldi mi kontrol eder.</small></div><button class="alkam-cache-close">×</button></div><div class="alkam-cache-body" id="alkamCacheBody"></div></div>';
