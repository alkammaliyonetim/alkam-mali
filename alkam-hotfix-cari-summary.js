// ALKAM Mali - Secili Cari Mini Ozet Kartlari
// Tablo verisini okumaz/yazmaz; sadece gorunen cari ekstre tablosundan ozet uretir.
(function(){
  'use strict';
  if(window.__ALKAM_CARI_SUMMARY_V1__) return;
  window.__ALKAM_CARI_SUMMARY_V1__ = true;
  ...
  function ensureStyle(){
    ...
    st.textContent = '#alkamCariMiniSummary{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:8px;margin:0 0 10px 0}' +
      ...
      '.alkam-cari-mini-card.risk{border-color:#fecaca;background:#fff7f7}.alkam-cari-mini-card.good{border-color:#bbf7d0;background:#f0fdf4}' +
      '@media(max-width:760px){#alkamCariMiniSummary{grid-template-columns:1fr 1fr}.alkam-cari-mini-card .v{font-size:13px}}';
  ...
  function buildSummary(){
    ...
    var allDates = [], tahakkukDates = [], tahsilatDates = [];
    ...
    var riskClass = (typeof gap === 'number' && gap >= 90) ? 'risk' : 'good';
    ...
    box.innerHTML = ''+
      ...
      '<div class="alkam-cari-mini-card '+riskClass+'"><div class="k">Tahsilatsız Gün</div><div class="v">'+riskText+'</div></div>'+
